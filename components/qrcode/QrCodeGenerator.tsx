...
export const ERROR_CORRECTION_LEVELS = {
  L: 'Low (7%) - Smaller QR codes',
  M: 'Medium (15%) - Balanced',
  Q: 'Quartile (25%) - Good for printing',
...
const useQRCodeGenerator = () => {
  ...
  const validateInput = useCallback((type: QrCodeType, value: string): string | null => {
    ...
    const validatePhone = (phone: string): boolean => /^[\+]?[1-9][\d]{0,19}$/.test(phone.replace(/[\s\-\(\)]/g, ''));

    switch (type) {
      case 'url':
        return value && !validateUrl(value) ? 'Invalid URL format (include http:// or https://)' : null;
      case 'email':
        return value && !validateEmail(value.split('?')[0].replace('mailto:', '')) ? 'Invalid email format' : null;
      case 'phone':
        return value && !validatePhone(value.replace('tel:', '')) ? 'Invalid phone number' : null;
      case 'sms':
        return value && !validatePhone(value.split(':')[1] || '') ? 'Invalid phone number' : null;
      case 'vcard':
        const emailMatch = value.match(/EMAIL:([^\n]*)/);
        const phoneMatch = value.match(/TEL;TYPE=WORK,VOICE:([^\n]*)/);
        const urlMatch = value.match(/URL:([^\n]*)/);
        if (emailMatch && emailMatch[1] && !validateEmail(emailMatch[1])) {
          return 'Invalid email format in vCard';
        }
        if (phoneMatch && phoneMatch[1] && !validatePhone(phoneMatch[1])) {
          return 'Invalid phone number in vCard';
        }
        if (urlMatch && urlMatch[1] && !validateUrl(urlMatch[1])) {
          return 'Invalid website URL in vCard';
        }
        return null;
      case 'wifi':
        return value.includes('S:') && !value.split(';')[1].replace('S:', '').trim() ? 'WiFi SSID is required' : null;
      default:
        return null;
    }
  }, []);
  ...
  const handleTypeChange = useCallback((type: QrCodeType) => {
    const value = type === QR_CODE_TYPES.URL ? 'https://example.com' : '';
    setState(prev => ({
      ...prev,
  ...
  const handleDataChange = useCallback(
    async (value: string, type: QrCodeType) => {
  ...
  return { state, setState, handleTypeChange, handleDataChange, validateInput };
};

// Components
const QrCodeTypeSelector: React.FC<{ selected: QrCodeType; onSelect: (type: QrCodeType) => void }> = React.memo(
  ({ selected, onSelect }) => {
    const types = [
      { id: QR_CODE_TYPES.URL, icon: Link, label: 'URL', description: 'Website links' },
      { id: QR_CODE_TYPES.TEXT, icon: Type, label: 'Text', description: 'Plain text' },
      { id: QR_CODE_TYPES.EMAIL, icon: Mail, label: 'Email', description: 'Email compose' },
      { id: QR_CODE_TYPES.PHONE, icon: Phone, label: 'Phone', description: 'Phone calls' },
      { id: QR_CODE_TYPES.SMS, icon: MessageSquare, label: 'SMS', description: 'Text messages' },
      { id: QR_CODE_TYPES.WIFI, icon: Wifi, label: 'WiFi', description: 'WiFi credentials' },
      { id: QR_CODE_TYPES.VCARD, icon: User, label: 'vCard', description: 'Contact info' },
    ];
          ...
          {types.map(type => (
            <button
              ...
              className={`flex flex-col items-center p-3 text-sm font-medium rounded-lg transition-all hover:scale-105 ${
                selected === type.id ? 'bg-primary text-white shadow-lg' : 'bg-gray-100 text-secondary hover:bg-gray-200'
              }`}
...
}> = React.memo(({ type, onChange, onValidationChange }) => {
...
}> = React.memo(({ options, setOptions, isOpen, setIsOpen }) => {
  return (
    <div className="mt-6 border-t pt-6">
      ...
      {isOpen && (
        <div className="mt-4 p-4 bg-gray-50 rounded-lg space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-secondary mb-2">Foreground Color</label>
              <div className="flex items-center gap-2">
                <input
                  type="color"
                ...
                <input
                  ...
                  placeholder="#000000"
                  aria-label="Foreground color input"
                />
              </div>
            </div>

            <div>
              <label className="block text-sm font-medium text-secondary mb-2">Background Color</label>
              <div className="flex items-center gap-2">
                <input
                  type="color"
          ...
          <div>
            <label className="block text-sm font-medium text-secondary mb-2">Size: {options.size}px</label>
            <input
              type="range"
              min="100"
              max="1000"
          ...
          <div>
            <label className="block text-sm font-medium text-secondary mb-2">Error Correction Level</label>
            <select
              value={options.level}
            ...
            >
              {Object.entries(ERROR_CORRECTION_LEVELS).map(([level, description]) => (
...
const BulkGenerationPanel: React.FC<{
  bulkData: BulkDataRow[];
  setBulkData: (data: BulkDataRow[]) => void;
  isProcessing: boolean;
  downloadProgress: number;
  onBulkGenerate: () => void;
  isOpen: boolean;
  setIsOpen: (open: boolean) => void;
}> = React.memo(({ bulkData, setBulkData, isProcessing, downloadProgress, onBulkGenerate, isOpen, setIsOpen }) => {
    ...
    if (file) {
      Papa.parse(file, {
        ...
        complete: (results) => {
          ...
          setBulkData(results.data as BulkDataRow[]);
        },
        error: (error) => {
  ...
  return (
    <div className="mt-6 border-t pt-6">
      <button
        ...
        onKeyDown={e => e.key === 'Enter' && setIsOpen(!isOpen)}
        className="w-full flex items-center justify-between p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors"
        aria-label="Toggle bulk generation panel"
        role="button"
        tabIndex={0}
      >
        <div className="flex items-center gap-2">
      ...
      {isOpen && (
        <div className="mt-4 p-4 bg-gray-50 rounded-lg space-y-4">
          <div className="text-sm text-secondary">
            ...
            <p className="mb-4">
              Required columns: <code className="bg-white px-1 rounded">filename</code> and{' '}
              <code className="bg-white px-1 rounded">value</code>
            </p>
          </div>
          ...
          {bulkData.length > 0 && (
            <div className="bg-white p-3 rounded-lg border">
              <p className="text-sm font-medium text-dark mb-2">✅ {bulkData.length} rows loaded successfully</p>
              <div className="max-h-32 overflow-y-auto">
                <div className="text-xs text-secondary">
                  ...
                  {bulkData.length > 3 && <div className="text-gray-400">... and {bulkData.length - 3} more rows</div>}
                </div>
              </div>
            </div>
          )}
          ...
          {isProcessing && (
            <div className="bg-white p-3 rounded-lg border">
              ...
              <div className="text-xs text-secondary mt-1">{downloadProgress.toFixed(0)}% complete</div>
            </div>
          )}
          ...
          <button
            ...
            aria-label="Generate and download bulk QR codes"
          >
            {isProcessing ? `Processing ${bulkData.length} QR Codes...` : `Generate & Download ${bulkData.length} QR Codes`}
          </button>
        </div>
      )}
    </div>
...
export const QrCodeGenerator: React.FC = () => {
  const { state, setState, handleTypeChange, handleDataChange, validateInput } = useQRCodeGenerator();
  ...
  const handleDownload = async (format: 'png' | 'svg' | 'pdf') => {
    try {
      ...
      if (typeof window !== 'undefined' && (window as any).trackQRDownload) {
        (window as any).trackQRDownload(format);
      }
    } catch (error) {
      setState(prev => ({
        ...prev,
  ...
  const generateBulkQRs = async () => {
    ...
    try {
      for (let i = 0; i < state.bulkData.length; i += batchSize) {
        const batch = state.bulkData.slice(i, i + batchSize);
        await Promise.all(
          batch.map(async (row, index) => {
            const qrValue = row.value || JSON.stringify(row);
            const filename = sanitizeFileName(row.filename || `qrcode_${i + index}`);
            const validationError = validateInput(state.options.type, qrValue);
            if (validationError) {
              throw new Error(`Invalid data for ${filename}: ${validationError}`);
            }
            ...
            // Note: In a real implementation, you'd need to render this to a string
            // For simplicity, we're generating a placeholder SVG
            const svgContent = `
        ...
        setState(prev => ({
          ...prev,
          downloadProgress: ((i + batchSize) / state.bulkData.length) * 100,
        }));
      ...
      if (typeof window !== 'undefined' && (window as any).trackBulkQRGeneration) {
        (window as any).trackBulkQRGeneration(state.bulkData.length);
      }
    } catch (error) {
      setState(prev => ({
        ...prev,
  ...
  const qrCode = useMemo(() => {
    return state.options.value && !state.validationError && !state.isGenerating ? (
      <QRCodeSVG
        value={state.options.value}
        size={Math.min(state.options.size, 300)}
        fgColor={state.options.fgColor}
        bgColor={state.options.bgColor}
        level={state.options.level}
        includeMargin={true}
    ...
    state.options.value,
    state.options.size,
    state.options.fgColor,
    state.options.bgColor,
    state.options.level,
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg overflow-hidden">
      <div className="p-6">
        <QrCodeTypeSelector selected={state.options.type} onSelect={handleTypeChange} />
        ...
        {(state.error || state.validationError) && (
          <div className="mb-4 p-3 bg-red-50 border border-red-200 rounded-lg flex items-start gap-2">
            ...
            <div>
              ...
              <p className="text-red-600 text-sm">{state.validationError || state.error}</p>
            </div>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="space-y-6">
            <QrCodeInput
              ...
              onChange={value => handleDataChange(value, state.options.type)}
              onValidationChange={error => setState(prev => ({ ...prev, validationError: error }))}
            />
          ...
          <div className="flex flex-col items-center space-y-6">
                ...
                {qrCode ? (
                  ...
                  >
                    <div className="text-center text-gray-500">
                      ...
                      <p>{state.isGenerating ? 'Generating QR code...' : 'Enter data to generate QR code'}</p>
                    </div>
                  </div>
                )}
              </div>
            ...
            <div className="w-full max-w-sm space-y-3">
              ...
              <div className="grid grid-cols-3 gap-2">
                <button
                  ...
                  disabled={!state.options.value || !!state.validationError || state.isGenerating}
            ...
            <div className="w-full max-w-sm">
              <p className="text-center text-sm font-semibold text-secondary mb-3">Share this tool!</p>
